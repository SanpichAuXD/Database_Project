...
router.put("/adminorg/update/:id/:status", async (req,res,_next) => {
    ...
    try{
        const [status, fields] = await pool.query(`update problem_info
                                                   set prob_status = ?
                                                   where prob_id = ?`, [req.params.status,req.params.id]);
    }
    catch(err){
        return console.log(err)
    }
...
router.put("/adminorg/:status", async (req, res) => {
    ...
    const [adminOrg, fields] = await pool.query(
        ...
        using(prob_id)
         ...
         FROM prob_image
         GROUP BY prob_id) AND prob_status = ?`,
         [status]
    );
    ...
    if(req.params.status === 'accept'){
        const [prob_accept, fields] = await pool.query(
            ...
            using(prob_id)
             WHERE img_id in (SELECT  MIN(img_id)
           FROM prob_image
           GROUP BY prob_id) AND prob_status NOT IN ('Declined','Verifying')
             `
        );
        const user = req.session.userID;
        const isAdmin = req.session.isAdmin;
        ...
        const [prob_decline, fields] = await pool.query(
            ...
            using(prob_id)
             WHERE img_id in (SELECT  MIN(img_id)
           FROM prob_image
           GROUP BY prob_id) AND prob_status = 'Declined'
             `
        );
        const user = req.session.userID;
        const isAdmin = req.session.isAdmin;
